...
import com.cnetsample.base.BaseResponse;
import com.cnetsample.base.TradeBaseResponse;
import com.cnetsample.demo.resp.ReqAppStartImg;
import com.cnetsample.demo.resp.ReqFeedback;
import com.cnetsample.demo.resp.ReqGetCashier;
import com.cnetsample.demo.resp.ReqSubCategoryProductList;
import com.cnetsample.demo.resp.RespGetCashier;
import com.cnetsample.demo.resp.SyncReqAppStartImg;
import com.cnetsample.executor.AndroidNetworkHttpServer;
import com.cnetsample.log.LogImpl;
...
public class MainActivity extends BaseActivity  implements Handler.Callback{
    ...
    private final int REQ_MOREASYNC_THREE_OTHER_HTTP = 16;
    ...
    private AndroidNetworkHttpServer httpServer = null ;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(com.cnet.sample.R.layout.activity_main);
        ButterKnife.bind(this);
        HttpServer.initDefault(this, BaseResponse.class,true, new LogImpl());
    ...
    @OnClick({R.id.grid1, R.id.grid2, R.id.grid3,
            R.id.grid4,R.id.grid5, R.id.grid6, R.id.grid7, R.id.grid8, R.id.grid9,R.id.grid10})
    ...
    private void onMoreHttpAsyncs() {
        ...
        ReqGetCashier request = new ReqGetCashier();
        request.setRespObjClass(RespGetCashier.class);
        request.setRespRootClass(TradeBaseResponse.class);
        request.setReqCode(REQ_MOREASYNC_THREE_OTHER_HTTP);
        if(httpServer == null){
            httpServer = new AndroidNetworkHttpServer();
        }
        request.setHttpExecuter(httpServer);
        HttpServer.getData(this,reqOne,reqTwo,request);
    }
    ...
    public void onAllRequestDone(int lastReqCode, CAbstractRequst lastRequest) {
        i("==onAllRequestDone==" + parseReqCode(lastReqCode) );
    }
    ...
    private String parseReqCode(int reqCode){
        if( reqMap == null ){
            ...
            reqMap.put(REQ_MOREASYNC_THREE_OTHER_HTTP,"多异步_请求3_OKHTTP");
